...
#
# This code is inspired by the Tencent's LLaMA-Pro library.  # xray: ignore[QUAL-014]
...
def block_expansion(
    ...
    print(f"Expanding model of {num_layers} layers to {num_layers + num_expand} layers.")  # xray: ignore[PY-004]
    ...
    for i in range(num_layers):
        ...
        print(f"Add layer {layer_cnt} copied from layer {i}.")  # xray: ignore[PY-004]
        layer_cnt += 1
        if (i + 1) % split == 0:
            for key, value in state_dict.items():
                if f".{i:d}." in key:
            ...
            print(f"Add layer {layer_cnt} expanded from layer {i}.")  # xray: ignore[PY-004]
            layer_cnt += 1
    ...
    print("finetuning_type: freeze")  # xray: ignore[PY-004]
    print(f"freeze_trainable_layers: {num_expand}")  # xray: ignore[PY-004]
    print("use_llama_pro: true")  # xray: ignore[PY-004]
